...
_sum = np.sum(df_submission['unsigned_percentage_return'].values)
...
for i in range(0, len(bins)):
    percentiles.append(df_submission['unsigned_percentage_return'].quantile(bins[i]))
percentiles[-1] += 0.5
for i in range(1, len(percentiles)):
    df_submission[f'Rank{i}'] = np.where((df_submission['unsigned_percentage_return'].values >= percentiles[i-1]) &\
                                         (df_submission['unsigned_percentage_return'].values < percentiles[i]), 1, 0)
